...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    ...
    model_name = Path(yaml_file).stem.replace("yolo", "inference") or "Model"
    LOGGER.info(f"{model_name} summary{fused}: {n_l} layers, {n_p} parameters, {n_g} gradients{fs}")
    return n_l, n_p, n_g, flops


def get_num_params(model):
    """Return the total number of parameters in a inference model."""
    return sum(x.numel() for x in model.parameters())


def get_num_gradients(model):
    """Return the total number of parameters with gradients in a inference model."""
...
def get_flops(model, imgsz=640):
    """Return a inference model's FLOPs."""
    if not thop:
