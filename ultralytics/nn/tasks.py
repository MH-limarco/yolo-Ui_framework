...
class BaseModel(nn.Module):
    """The BaseModel class serves as a base class for all the models in the Ultralytics inference family."""
...
class DetectionModel(BaseModel):
    ...
    def _clip_augmented(self, y):
        """Clip inference augmented inference tails."""
...
class ClassificationModel(BaseModel):
    ...
    def reshape_outputs(model, nc):
        ...
        if isinstance(m, Classify):  # inference Classify() head
...
class Ensemble(nn.ModuleList):
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        """Function generates the inference network's final layer."""
...
def torch_safe_load(weight):
    ...
    if not isinstance(ckpt, dict):
        # File is likely a inference instance saved with i.e. torch.save(model, "saved_model.pt")
        LOGGER.warning(
            f"WARNING ⚠️ The file '{weight}' appears to be improperly saved or formatted. "
            f"For optimal results, use model.save('filename.pt') to correctly save inference models."
        )
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    """Parse a inference model.yaml dictionary into a PyTorch model."""
    import ast
...
def yaml_model_load(path):
    ...
    if path.stem in (f"yolov{d}{x}6" for x in "nsmlx" for d in (5, 8)):
        ...
        LOGGER.warning(f"WARNING ⚠️ Ultralytics inference P6 models now use -p6 suffix. Renaming {path.stem} to {new_stem}.")
...
def guess_model_scale(model_path):
    """
    Takes a path to a inference model's YAML file as input and extracts the size character of the model's scale. The function
    uses regular expression matching to find the pattern of the model scale in the YAML file name, which is denoted by
    n, s, m, l, or x. The function returns the size character of the model scale as a string.

    Args:
        model_path (str | Path): The path to the inference model's YAML file.
